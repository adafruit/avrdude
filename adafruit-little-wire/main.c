...
-----------------------------------------------------------
 ...
 - Modified by: Frank Zhao, June 2014, for Adafruit Industries
 -----------------------------------------------------------
...
enum
{
...
#ifdef ENABLE_ADDITIONAL_REQUESTS
	...
	USBTINY_FLASH_READ_FAR = 35,	// read flash (wIndex:address)
	USBTINY_FLASH_WRITE_FAR = 36,	// write flash (wIndex:address, wValue:timeout)
};
...
#if defined(COMPILE_FOR_PRODUCTION_ON_ATTINYn5)
//
...
#elif defined(COMPILE_FOR_TESTING_ON_ATMEGAnn8)
...
static	uint32_t	address;	// read/write address
static	unsigned	timeout;	// write timeout in usec
static	uchar		prev_req;	// the last bRequest
...
// ----------------------------------------------------------------------
	...
	if	( req == USBTINY_POWERDOWN )
	{
		...
		PORT &= ~(RESET_MASK | MISO_MASK | MOSI_MASK | SCK_MASK);
		DDR &= ~(RESET_MASK | MISO_MASK | MOSI_MASK | SCK_MASK);
		return 0;
	}
	if	( req == USBTINY_SPI )
	{
	...
	if	( req == USBTINY_FLASH_WRITE_FAR )
	{
		address <<= 1;
		cmd0 = 0x40;
		return 0xff;	// data will be received by usb_out()
